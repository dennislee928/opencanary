...
class CanaryService(object):
    ...
    def resource_dir(klass):
        ...
        # actually CanaryService is in opencanary/modules, so the root is __file__/..
        # Let's properly resolve this relative to opencanary
    ...
    def log(self, logdata, **kwargs):
        ...
        """
        data = {"logtype": self.logtype, "logdata": logdata}
    ...
    def getService(self):
        ...
        err = (
            "The class %s does not inherit from either Factory or DatagramProtocol."
            % (self.__class__.__name__)
        )
        raise Exception(err)
...
if sys.platform.startswith("linux"):  # noqa: C901
    ...
    class FileSystemWatcher(object):
        ...
        def reopenFiles(self, skipToEnd=True):
            ...
            try:
                self.notifier.watch(
                    filepath.FilePath(self.path), callbacks=[self.onChange]
                )
            except INotifyError:
                self.notifier.watch(
                    filepath.FilePath(self.log_dir),
                    mask=IN_CREATE,
